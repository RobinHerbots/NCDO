...
namespace NCDO.Interfaces
{
    public interface ICDOSession : IDisposable
    {
        ...
        /// </summary>
        Task<SessionStatus> Login(CancellationToken cancellationToken = default(CancellationToken), bool fromRequest = false);
        ...
        (Service service, Resource resource) VerifyResourceName(string resource);
        Operation VerifyOperation(string resource, string operation,
            OperationType operationType = OperationType.Invoke);
        string DetermineMainTable(string resource);
        string DeterminePrimaryKey(string resource, string tableName);
